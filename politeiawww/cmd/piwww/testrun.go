...
func proposalSetStatus(token, status, reason string, unvetted bool) error {
	pssc := proposalStatusSetCmd{
		Unvetted: unvetted,
	}
	pssc.Args.Token = token
	pssc.Args.Status = status
...
func proposalCensor(token, reason string, unvetted bool) error {
	const proposalStatusCensored = "censored"
	err := proposalSetStatus(token, proposalStatusCensored, reason, unvetted)
	if err != nil {
		return err
	}
	return nil
...
func proposalAbandon(token, reason string) error {
	const proposalStatusAbandoned = "abandoned"
	err := proposalSetStatus(token, proposalStatusAbandoned, reason, false)
	if err != nil {
		return err
	}
	return nil
...
func testProposalRoutes(admin testUser) error {
	...
	err = proposalCensor(censoredToken1, reason, true)
	if err != nil {
		return err
	}
	...
	err = proposalCensor(censoredToken2, reason, false)
	if err != nil {
		return err
	}
